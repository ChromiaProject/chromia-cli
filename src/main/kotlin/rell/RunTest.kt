...
private fun CliktCommand.compilerOption() =
    option("-co", "--compiler-properties", help = "File path with compiler settings" )
        .default("compilerOptions.properties")
...
class RunTest: CliktCommand(help= "Run tests in working directory") {
    ...
    private val coProperties by compilerOption()
    ...
    private fun createContext(app: R_App): Context {
        val properties = Properties()
        val stream = this::class.java.classLoader.getResourceAsStream(coProperties)
        if (stream == null) {
            throw RellCliErr("Must specify a resource file for the compiler options")
        } else {
            properties.load(stream)
            val compilerOptions = CompilerOptions(properties, rellVersion).getCompilerOptions()
