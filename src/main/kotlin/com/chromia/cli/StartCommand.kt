...
class StartCommand : CliktCommand(help = "Starts a node") {
    ...
    private val blockchains by option(help = "Only start specified blockchains")
            .multiple()
    ...
    private fun startPostchainNode() {
        ...
        val configsToStart = if (blockchains.isEmpty()) configsToAdd else configsToAdd.filter { blockchains.contains(it.key.name)  }
