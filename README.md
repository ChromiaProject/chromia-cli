...
You can set a path for it or specify a different Project Settings file.
```shell
chr start --settings chromia.yml
```
...
You can set a path for it or specify a different Project Settings file.
```shell
chr deploy --settings chromia.yml --target devnet2 --secret .secret
```

If you have multiple chains and only want to deploy a specific one.
```shell
chr deploy --settings chromia.yml --target devnet2 --blockchain hello --secret .secret
```
### Query
