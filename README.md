...
- To be able to test/run your dapp locally, you need to have a postgres accessible see [here](https://docs.chromia.com/getting-started/dev-setup/database-setup) 
...
./.chromia/config
```
are used to configure how the cli works. As noted, it can be set on global level or on local(project) level. This is where you store your keypair in the format:
```properties
pubkey=
privkey=
```
...
The project settings file is where most of the configurations for your dapp is found, by default it is named `chromia.yml`
...
### Node Start
Start command is used to start a test node with you applications running on it. It will start all the blockchains under the 
